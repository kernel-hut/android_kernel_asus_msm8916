...
int Olivia_Read_Calibration_Value_From_File(struct seq_file *vfile, uint16_t *cal_data){
	...
	LOG_Handler(LOG_CDBG,"Cal data: %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x\n",
		cal_data[0], cal_data[1], cal_data[2], cal_data[3], cal_data[4],
		cal_data[5], cal_data[6], cal_data[7], cal_data[8], cal_data[9],cal_data[10], cal_data[11],
		cal_data[12], cal_data[13], cal_data[14], cal_data[15], cal_data[16]);	
...
int	Read_Range_Data(struct msm_laser_focus_ctrl_t *dev_t){
	...
	if(RawRange&VALID_DATA){
		...
		if((error_status==NO_ERROR)){
			...
			if(Range == 2047){
				LOG_Handler(LOG_DBG, "%s: read range is 2047, not accept\n", __func__);
				Range = OUT_OF_RANGE;
...
int Olivia_device_read_range(struct msm_laser_focus_ctrl_t *dev_t)
{
	...
	if(status != 0) 
	{
		LOG_Handler(LOG_ERR, "%s: Verify_MCPU_On_by_Time Exit with Error: %d\n", __func__, status);	
		goto read_err;
	}
	...
	/* Trigger single measure */
       status = CCI_I2C_WrWord(dev_t, COMMAND_REGISTER, (SINGLE_MEASURE|VALIDATE_CMD));
       if (status < 0){
				LOG_Handler(LOG_ERR, "%s: CCI_I2C_WrWord COMMAND_REGISTER Exit with Error: %d\n", __func__, status);	
            	return status;
       }
	status = Verify_Range_Data_Ready(dev_t);
	if(status != 0) 
	{
		LOG_Handler(LOG_ERR, "%s: Verify_Range_Data_Ready Exit with Error: %d\n", __func__, status);	
		goto read_err;	
	}		

	Range = Read_Range_Data(dev_t);
	if(Range < 0){
		LOG_Handler(LOG_ERR, "%s: Read_Range_Data Exit with Error: %d\n", __func__, Range);	
		status = Range;
		goto read_err;
	}
