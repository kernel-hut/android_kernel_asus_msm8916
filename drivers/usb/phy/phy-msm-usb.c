...
//ASUS_BSP+++ ShowWang "[ZC550KL][USB][NA][Fix] recheck vbus state plugout ID cable 0.5s later"
...
static int g_skipped_5v=0;
static int g_carkit_in=0;
/* ASUS_BSP--- Landice "[ZE500KL][USBH][NA][fix] Support host charging for stress test" */
...
/* ASUS_BSP+++ Landice "[ZE500KL][USBH][NA][fix] Support host charging for stress test" */
		...
		if (g_skipped_5v && is_usb_chg_plugged_in()) {
...
static ssize_t asus_otg_proc_host_charging_mode_write(struct file *file, const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, "ac", 2)) {
		...
		g_skipped_5v = 1;
		printk("[usb_otg] Set host charging mode to g_skipped_5v = 1\n");
	} else {
		return -EINVAL;
	}
...
struct switch_dev asus_msm_otg_poweron;

static ssize_t asus_msm_otg_poweron_switch_name(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "otg_poweron\n");
}

static ssize_t asus_msm_otg_poweron_switch_state(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "%d\n", g_vbus_is_on);
}

static int AsusOtgPowerOnNotifyInitialize(void)
{
	int ret = 0;
	printk("[USB] %s: register otg vbus switch %d\n", __FUNCTION__, ret);
	asus_msm_otg_poweron.name = "otg_poweron";
	asus_msm_otg_poweron.print_state = asus_msm_otg_poweron_switch_state;
	asus_msm_otg_poweron.print_name = asus_msm_otg_poweron_switch_name;
	ret = switch_dev_register(&asus_msm_otg_poweron);
	if (ret < 0) {
...
static void asus_otg_chg_usb_work(struct work_struct *w)
{
	...
	if(test_bit(B_SESS_VLD, &motg->inputs)){
		...
		if(g_ASUS_bootmode == USER_MODE) {
			printk("[usb_otg] %s %d  PORTSC: %08x \n", __func__, __LINE__, readl_relaxed(USB_PORTSC));
		}
...
//ASUS_BSP--- ShowWang "set otg current as requirement"
			if(!g_otg_poweron) { //ASUS_BSP: ShowWang "Enable otg poweron switching"
				asus_otg_set_charger(DISABLE_5V);
			}
		}
		dev_info(phy->dev, "%s()---\n", __func__);
...
//ASUS_BSP+++ ShowWang "support charging for Iphone when otg poweron"
		...
		{
			g_apple_attach = 1;
			printk("[usb_otg] otg keep poweron and Apple, Inc, Insert %s VID = %x \n", __func__,udev->descriptor.idVendor);
		} else {
			g_apple_attach = 0;
		}
...
//ASUS_BSP+++ Landice "[ZE500KL][USBH][NA][other] Add debug file for checking VBUS output status"
	...
	g_vbus_is_on = on;
	switch_set_state(&asus_msm_otg_poweron, g_vbus_is_on);
	printk("[usb_otg] %s - send vbus switch state %d\n", __func__, g_vbus_is_on);
//ASUS_BSP--- Landice "[ZE500KL][USBH][NA][other] Add debug file for checking VBUS output status"
}
//ASUS_BSP--- Landice "[ZE500KL][USBH] Enable/Disable VBUS output on ID events"
...
static int msm_otg_id_state(void)
{
	struct msm_otg *motg = the_msm_otg;
	int id_state = 0;
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_DETECTED:
		...
		if(first_detect)
		{
			if (motg->chg_type == USB_DCP_CHARGER)
			{
				printk("chg typ detected as DCP first time, delay 0.5s \n");
				delay=500 * HZ/1000;
			} else
			{
				printk("chg typ detected as not DCP first time, delay 1s \n");
				delay=1000 * HZ/1000;
			}
			motg->chg_state = USB_CHG_STATE_DCD_DONE;
...
/*			if(is_usb_chg_plugged_in() && msm_otg_id_state()) {
...
//ASUS_BSP--- Landice "[ZE500KL][USBH][NA][Spec] Set asus charger upon charger type detection"
	...
	case OTG_STATE_A_WAIT_BCON:
		if ((test_bit(ID, &motg->inputs) &&
			...
			if (test_bit(ID_A, &motg->inputs))
				...
				if( !g_otg_poweron || (g_otg_poweron && g_otg_state_in) ){//ASUS_BSP: ShowWang "Enable otg poweron switching"
					msm_hsusb_vbus_power(motg, 0);
				}
			}
			otg->phy->state = OTG_STATE_A_WAIT_VFALL;
	...
	case OTG_STATE_A_HOST:
		if ((test_bit(ID, &motg->inputs) &&
			...
			otg->phy->state = OTG_STATE_A_WAIT_VFALL;
			msm_otg_start_host(otg, 0);
			if (!g_otg_poweron && !test_bit(ID_A, &motg->inputs))//ASUS_BSP: ShowWang "Enable otg poweron switching"
				msm_hsusb_vbus_power(motg, 0);
			//ASUS_BSP+++ Show_Wang "skip otg detect waiting time"
			msm_otg_start_timer(motg, 0, A_WAIT_VFALL);
			//ASUS_BSP--- Show_Wang "skip otg detect waiting time"
		} else if (!test_bit(A_VBUS_VLD, &motg->inputs)) {
			pr_debug("!a_vbus_vld\n");
			clear_bit(B_CONN, &motg->inputs);
...
//ASUS_BSP+++ Landice "[ZE500KL][USBH][Spec] Register early suspend notification for none mode switch"
	if( (motg->otg_mode != USB_AUTO && motg->otg_mode != USB_PERIPHERAL) || motg->host_mode || !msm_otg_id_state()) {
		printk("[usb_otg] Not in auto mode, skip set vbus state, mode%d\n", motg->otg_mode);
		return;
	}
//ASUS_BSP--- Landice "[ZE500KL][USBH][Spec] Register early suspend notification for none mode switch"
...
//ASUS_BSP--- Landice "[ZE500KL][USBH][NA][fix] Workaround for factory issue that receive same ID events continuously"
	...
	if(!id_state && test_bit(B_SESS_VLD, &motg->inputs)) {
		g_carkit_in =1;
		old_id_state = id_state;
		printk("[usb_otg] %s, carkit plugin ignore ID detection \n", __func__);
		return;
	}

	if(id_state && g_carkit_in && !test_bit(B_SESS_VLD, &motg->inputs)){
		g_carkit_in =0;
		old_id_state = id_state;
		printk("[usb_otg] %s, carkit plugout ignore ID detection \n", __func__);
		return;
	}
//ASUS_BSP+++ Landice "[ZE500KL][USBH][Spec] Register early suspend notification for none mode switch"
	if (id_state) {
		...
		//ASUS_BSP--- ShowWang "[ZC550KL][USB][NA][Fix] recheck vbus state plugout ID cable 0.5s later"
		//ASUS_BSP+++ ShowWang "Enable otg poweron switching"
/*		if(g_otg_poweron && motg->otg_mode == USB_AUTO && g_screen_off)
		{
			printk("[usb_otg] %s disable 5V when screen off\n", __func__);
			asus_otg_set_charger(DISABLE_5V);
			g_vbus_is_on=false;
			switch_set_state(&asus_msm_otg_poweron, g_vbus_is_on);
			old_id_state = id_state;
...
/*		if(g_otg_poweron && motg->otg_mode == USB_AUTO && g_screen_off)
		{
			printk("[usb_otg] %s enable 5V when screen off \n", __func__);
...
//ASUS_BSP--- ShowWang "add otg check at boot"
	...
	AsusOtgPowerOnNotifyInitialize();
	return 0;
